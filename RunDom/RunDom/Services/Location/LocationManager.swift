...
final class LocationManager: NSObject, ObservableObject {
    ...
    func stopTracking() {
        ...
        lastValidLocation = nil
        AppLogger.location.info("Location tracking stopped")
    }
    ...
    func saveLastKnownLocation() {
        ...
        UserDefaults.standard.set(location.coordinate.latitude, forKey: AppConstants.UserDefaultsKeys.lastKnownLatitude)
        UserDefaults.standard.set(location.coordinate.longitude, forKey: AppConstants.UserDefaultsKeys.lastKnownLongitude)
    }

    var lastKnownCoordinate: CLLocationCoordinate2D? {
        let lat = UserDefaults.standard.double(forKey: AppConstants.UserDefaultsKeys.lastKnownLatitude)
        let lon = UserDefaults.standard.double(forKey: AppConstants.UserDefaultsKeys.lastKnownLongitude)
...
extension LocationManager: CLLocationManagerDelegate {
        ...
        Task { @MainActor in
            for location in locations {
                ...
                if let last = lastValidLocation, hasGPSGap(from: last, to: location) {
                    AppLogger.location.warning("GPS gap detected: \(location.timestamp.timeIntervalSince(last.timestamp))s")
                    gpsSignalLost = true
    ...
    nonisolated func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
        Task { @MainActor in
            ...
            AppLogger.location.info("Authorization changed: \(manager.authorizationStatus.rawValue)")
        }
    }
}
